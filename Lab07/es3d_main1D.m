function [ k_iter ]=es3d_main1D(TestName,nRef)
%==========================================================================
...
[ uh, ~, ~, k_iter] = pcg( A, b, tol, nmax );

% stimatore del metodo del pcg

c_cg = ( sqrt(num_cond)-1 ) / ( sqrt(num_cond)+1 );

stm_cg = @(kk) ( 2 * c_cg .^kk ) ./ ( 1 + c_cg.^(2*kk) );

% stimatore del metodo del gradiente

stm_gr = @(kk) ( (num_cond-1)/(num_cond+1) ).^kk;
...
figure()
plot( 1: k_iter, stm_cg(1: k_iter), '-o' );
hold on
plot( 1: k_iter, stm_gr(1: k_iter), '-o' );
legend('PCG', 'Gradiente');
title(['Fattore abbattimento errore (nRef=', num2str(nRef), ')']);
xlabel('iterazioni');
hold off;
